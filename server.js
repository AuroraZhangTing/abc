...
function getUserById(userId) {
  return db.prepare('SELECT id, username, real_name, role, status FROM users WHERE id = ?').get(userId);
}
function listUsers() {
  return db.prepare('SELECT id, username, real_name, role, status, created_at, updated_at FROM users ORDER BY id').all();
}
function normalizeRole(role) {
  return ['admin', 'executor', 'viewer'].includes(role) ? role : 'viewer';
}
  ...
  if (pathname.startsWith('/api/')) {
    try {
      ...
      if (req.method === 'POST' && pathname === '/api/setup/admin') {
        ...
        const result = db.prepare("INSERT INTO users (username, real_name, role, password_hash, status, created_at, updated_at) VALUES (?, ?, 'admin', '', 1, ?, ?)")
      ...
      if (req.method === 'POST' && pathname === '/api/users') {
        ...
        const body = await parseBody(req);
        if (!body.username || !body.real_name) {
          return json(res, 400, { message: '账号和姓名不能为空' });
        }
        const role = normalizeRole(body.role);
        try {
          const result = db.prepare("INSERT INTO users (username, real_name, role, password_hash, status, created_at, updated_at) VALUES (?, ?, ?, '', ?, ?, ?)")
            .run(body.username.trim(), body.real_name.trim(), role, Number(body.status ?? 1), now(), now());
          return json(res, 201, { user: getUserById(Number(result.lastInsertRowid)) });
        } catch (error) {
          return json(res, 400, { message: '账号已存在，无法重复创建' });
        }
      }
      ...
      if (userMatch && req.method === 'PUT') {
        ...
        const status = Number(body.status ?? existing.status ?? 1);
        db.prepare('UPDATE users SET username = ?, real_name = ?, role = ?, status = ?, updated_at = ? WHERE id = ?')
          .run((body.username || existing.username).trim(), (body.real_name || existing.real_name).trim(), role, status, now(), userId);
        return json(res, 200, { user: getUserById(userId) });
      }
      ...
      if (userMatch && req.method === 'DELETE') {
        ...
        const existing = getUserById(userId);
        if (!existing) return notFound(res);
        const activeAdmins = db.prepare("SELECT COUNT(*) AS count FROM users WHERE role = 'admin' AND status = 1").get().count;
